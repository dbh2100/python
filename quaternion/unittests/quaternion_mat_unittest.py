...
class QuaternionMatrixTestCase(unittest.TestCase):
    ...
    def test_preserve_add(self):
        self.assertQEqual(matrix_to_quaternion(self.c1 + self.c2), self.q1 + self.q2)
        self.assertQEqual(matrix_to_quaternion(self.r1 + self.r2), self.q1 + self.q2)

    def test_preserve_mult(self):
        try:
            self.assertQEqual(matrix_to_quaternion(numpy.matmul(self.c1, self.c2)), self.q1 * self.q2)
            self.assertQEqual(matrix_to_quaternion(numpy.matmul(self.r1, self.r2)), self.q1 * self.q2)
